...
contains
    ...
    subroutine initialize_true_df_packed_factors()
        ...
        if (allocated(pair_mu)) deallocate(pair_mu)
        if (allocated(pair_nu)) deallocate(pair_nu)
        if (allocated(true_df_B)) deallocate(true_df_B)
        true_df_ready = .false.
        true_df_packed_ready = .true.
        ...
        deallocate(buf2c, buf3c, metric, rhs)
        deallocate(ao_loc_all, atm_all, bas_all, env_all)
    ...
    subroutine contract_jk_true_df_blocked(P, J, K, block_size)
        ...
        real(c_double), allocatable :: Bblk(:,:,:), Jblk(:,:), Kblk(:,:)
        ...
        allocate(Bblk(nao, nao, bs), Jblk(nao,nao), Kblk(nao,nao))
            ...
            do q = 1, qb
                ...
                do mu = 1, nao
                    do nu = 1, mu
                        ipair = ipair + 1
                        if (ipair > npair) exit
                        Bblk(mu,nu,q) = true_df_Bp(qq,ipair)
                        Bblk(nu,mu,q) = true_df_Bp(qq,ipair)
                    end do
                end do
            end do

            call contract_jk(Bblk(:,:,1:qb), qb, P, Jblk, Kblk)
            J = J + Jblk
            K = K + Kblk
        end do

        deallocate(Bblk, Jblk, Kblk)
